            ...
            <form method="post" action="settings.php#alert-settings" onsubmit="return submitForm()">
                <div id="alert-settings">
                    ...
                    <div class="mb-6">
                        ...
                        <input type="hidden" id="alert_msg" name="alert_msg" value="<?php echo htmlspecialchars($row['alert_msg'] ?? '', ENT_QUOTES); ?>">
                    </div>

                    <div class="mb-6">
                        <label for="alert_activation" class="block text-sm font-medium text-gray-400 mb-2">Activer l'Alerte :</label>
                        <input type="checkbox" id="alert_activation" name="alert_activation" value="1" <?php echo ($row["alert_activation"] ?? 0) == 1 ? 'checked' : ''; ?>>
                    </div>

                    <div class="mb-6">
                        <label for="alert_scroll" class="block text-sm font-medium text-gray-400 mb-2">Activer le défilement :</label>
                        <input type="checkbox" id="alert_scroll" name="alert_scroll" value="1" <?php echo ($row["alert_scroll"] ?? 0) == 1 ? 'checked' : ''; ?>>
                    </div>
                </div>
...
<script>
    var quill = new Quill('#editor-container', {
        ...
        placeholder: 'Écrivez votre message d\'alerte ici...'
    });
    ...
    function submitForm() {
        ...
        document.getElementById('alert_msg').value = content;
        return true;
    }
